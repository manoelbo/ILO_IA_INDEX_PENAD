...
def archive_uncompressed_bytes(path: Path) -> int:
    try:
        with py7zr.SevenZipFile(path, mode="r") as archive:
            members = archive.list()
        return max(
            (
                int(member.uncompressed)
                for member in members
                if member.uncompressed is not None
            ),
            default=0,
        )
...
def build_movements(
    ...
    *,
    ...
    start_free_bytes = shutil.disk_usage(building_dir).free
    minimum_free_bytes = start_free_bytes
    ...
    try:
        for index, record in enumerate(records, start=1):
            ...
            peak_scratch_bytes = max(
                peak_scratch_bytes,
                archive_uncompressed_bytes(archive_path),
            )
            print(
            ...
            minimum_free_bytes = min(
                minimum_free_bytes,
                shutil.disk_usage(building_dir).free,
            )
            del frame, prepared
            gc.collect()
    finally:
    ...
    metrics = {
        ...
        "observed_free_space_delta_bytes": (
            start_free_bytes - minimum_free_bytes
        ),
