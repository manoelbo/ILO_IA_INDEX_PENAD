...
SECTOR_SUPPORT_FIELDS = (
    ...
    "coexistence_bytes",
    "coexistence_sha256",
    "coexisting_treated_control_cells",
    "coexisting_treated_control_share",
    "divisions_including_undocumented",
    "duplicate_cells",
    "max_admission_wage",
    "max_separation_wage",
    ...
    "sector_panel_sha256",
    "sections",
    "undocumented_cnae_rows",
    "undocumented_cnae_signed_weight",
)
...
def _load_or_fetch_registered_source(
    *,
    raw_path: Path,
    manifest: dict[str, dict[str, Any]],
    manifest_key: str,
    ...
    if not isinstance(entry, dict):
        raise RuntimeError(
            f"Vintage manifest is missing registered source: {manifest_key}"
        )
    if entry.get("url") != expected_url:
        raise RuntimeError(
            f"Registered URL mismatch for {manifest_key}: {entry.get('url')}"
        )
    ...
    if not isinstance(expected_bytes, int) or not isinstance(
        ...
        raise RuntimeError(
            f"Incomplete size/hash contract for registered source: {manifest_key}"
        )

    reused = raw_path.is_file()
    payload = raw_path.read_bytes() if reused else fetch(expected_url)
    observed_sha256 = hashlib.sha256(payload).hexdigest()
    if len(payload) != expected_bytes or observed_sha256 != expected_sha256:
        raise RuntimeError(
            f"Registered source mismatch for {manifest_key}: expected "
            f"{expected_bytes} bytes/{expected_sha256}, observed "
            f"{len(payload)} bytes/{observed_sha256}"
        )
    if not reused:
        _atomic_bytes(raw_path, payload)
    return payload, entry, reused
...
def freeze_ipca(
    ...
    *,
    ...
    source_key = f"ipca/{raw_path.name}"
    payload, source_entry, reused = _load_or_fetch_registered_source(
        raw_path=raw_path,
        manifest=manifest,
        manifest_key=source_key,
        expected_url=IPCA_URL,
        fetch=fetch,
    )
    ...
    generated_at = manifest.get("derived/ipca_mensal.parquet", {}).get(
        "generated_at", source_entry.get("accessed_at")
    )
...
def freeze_cnae(
    ...
    *,
    ...
    manifest = _read_manifest(manifest_path)
    source_key = f"cnae/{raw_path.name}"
    payload, source_entry, reused = _load_or_fetch_registered_source(
        raw_path=raw_path,
        manifest=manifest,
        manifest_key=source_key,
        expected_url=CNAE_URL,
        fetch=fetch,
    )
    ...
    dictionary_key = f"cnae/{dictionary_path.name}"
    generated_at = manifest.get(dictionary_key, {}).get(
        "generated_at", source_entry.get("accessed_at")
    )
    manifest[dictionary_key] = {
...
def write_sector_support(metrics: dict[str, Any], path: Path) -> None:
    """Serialize sector support in the signed reference field order."""
    expected = set(SECTOR_SUPPORT_FIELDS)
    observed = set(metrics)
    if observed != expected:
        raise RuntimeError(
            "Sector support fields differ from the signed contract; "
            f"missing={sorted(expected - observed)}, "
            f"extra={sorted(observed - expected)}"
        )
    ordered = {field: metrics[field] for field in SECTOR_SUPPORT_FIELDS}
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_suffix(f"{path.suffix}.tmp")
    temporary.write_text(
...
def main() -> int:
    ...
    if args.command in {"build-sector", "all-sector"}:
        ...
        write_sector_support(metrics, args.sector_support)
        print(json.dumps(metrics, sort_keys=True))
    return 0
