...
def read_data(input_path: Path) -> pd.DataFrame:
    ...
    return canonicalize_analytic_order(frame)


def canonicalize_analytic_order(frame: pd.DataFrame) -> pd.DataFrame:
    """Make all downstream floating reductions independent of source order."""
