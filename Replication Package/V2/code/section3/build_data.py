...
PNAD_MANIFEST_FILENAME = "manifest.json"
ILO_FILENAME = "Final_Scores_ISCO08_Gmyrek_et_al_2025.xlsx"
PNAD_SOURCE_TABLE = "basedosdados.br_ibge_pnadc.microdados"
PNAD_SOURCE_URL = (
    "https://basedosdados.org/dataset/"
    "2d5c857a-0e9a-4d3f-8a1e-0f39f70a5c81"
)
PNAD_TERMS_URL = "https://www.ibge.gov.br/acesso-informacao/dados-abertos.html"
...
def write_pnad_source_manifest(
    raw_dir: Path,
    *,
    rows: int,
    queried_at: str,
) -> Path:
    source = raw_dir / PNAD_FILENAME
    manifest = {
        "files": {
            PNAD_FILENAME: {
                "bytes": source.stat().st_size,
                "columns": list(PNAD_COLUMNS),
                "encoding": "binary Parquet with Zstandard compression",
                "rows": int(rows),
        ...
        "query": pnad_query(),
        "source_table": PNAD_SOURCE_TABLE,
        "source_url": PNAD_SOURCE_URL,
        "status": "complete",
        "terms_url": PNAD_TERMS_URL,
        "vintage": f"{EXPECTED_YEAR}Q{EXPECTED_QUARTER}",
    }
    destination = raw_dir / PNAD_MANIFEST_FILENAME
...
def validate_pnad_source(raw_dir: Path) -> dict[str, Any]:
    source = raw_dir / PNAD_FILENAME
    manifest_path = raw_dir / PNAD_MANIFEST_FILENAME
    if not source.is_file() or not manifest_path.is_file():
        raise FileNotFoundError(
            "A cached Section 3 PNAD source requires both the Parquet file "
            f"and its manifest: {raw_dir}"
        )
    ...
    if (
        ...
        or manifest.get("source_table") != PNAD_SOURCE_TABLE
        or manifest.get("vintage") != f"{EXPECTED_YEAR}Q{EXPECTED_QUARTER}"
    ):
        raise ValueError("Section 3 PNAD source manifest contract mismatch")
    files = manifest.get("files")
    entry = files.get(PNAD_FILENAME) if isinstance(files, dict) else None
    if not isinstance(entry, dict):
        raise ValueError("Section 3 PNAD source is absent from its manifest")
    if source.stat().st_size != int(entry.get("bytes", -1)):
        raise ValueError("Section 3 PNAD source byte-size mismatch")
    if sha256_file(source) != entry.get("sha256"):
        raise ValueError("Section 3 PNAD source SHA-256 mismatch")
    if entry.get("columns") != list(PNAD_COLUMNS):
        raise ValueError("Section 3 PNAD source schema contract mismatch")
    return manifest
...
def load_pnad(raw_dir: Path, billing_project: str | None) -> pd.DataFrame:
    ...
    if local_path.exists():
        manifest = validate_pnad_source(raw_dir)
        frame = pd.read_parquet(local_path)
        validate_pnad_period(frame)
        expected_rows = int(manifest["files"][PNAD_FILENAME]["rows"])
        if len(frame) != expected_rows:
            raise ValueError(
                "Section 3 PNAD source row-count mismatch: "
    ...
    try:
        ...
        raise RuntimeError(
            "Full Section 3 acquisition requires `uv sync --frozen`."
        ) from exc
...
def resolve_ilo_path(raw_dir: Path) -> Path:
    candidates = (
        raw_dir / ILO_FILENAME,
        raw_dir.parent / "crosswalk" / ILO_FILENAME,
    )
    ...
    raise FileNotFoundError(
        f"Missing ILO workbook. Expected one of: {expected}. "
        "See README.md for the exact source."
    )


def load_ilo(raw_dir: Path) -> pd.DataFrame:
    path = resolve_ilo_path(raw_dir)
...
def apply_hierarchical_crosswalk(
    ...
    total_weight = stable_float_sum(frame["peso"])
    for digits in (4, 3, 2, 1):
        label = f"{digits}-digit"
        mask = frame["match_level"].eq(label)
        coverage[f"rows_{digits}_digit"] = int(mask.sum())
        coverage[f"population_{digits}_digit"] = stable_float_sum(
            frame.loc[mask, "peso"]
        )
    unmatched = frame["exposure_score"].isna()
    coverage["rows_unmatched"] = int(unmatched.sum())
    coverage["population_unmatched"] = stable_float_sum(
        frame.loc[unmatched, "peso"]
    )
    coverage["population_total"] = total_weight
    return frame, coverage


def stable_float_sum(values: pd.Series) -> float:
    """Return an order-independent sum for published build diagnostics."""
    numeric = pd.to_numeric(values, errors="raise").dropna().astype(float)
    return float(math.fsum(numeric.tolist()))


def write_canonical_build_diagnostics(source: Path, destination: Path) -> None:
    """Write a stable public receipt from legacy or newly built diagnostics."""
    with source.open(encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if not rows or tuple(rows[0]) != ("metric", "value"):
        raise ValueError(f"Invalid Section 3 build diagnostics: {source}")
    if len({row["metric"] for row in rows}) != len(rows):
        raise ValueError(f"Duplicate Section 3 build diagnostic: {source}")

    normalized: list[tuple[str, str]] = []
    for row in rows:
        metric = row["metric"]
        value = row["value"]
        if metric.startswith("population_") or metric == "final_population":
    ...
    *,
    ...
    diagnostics: dict[str, Any] = {
        ...
        **coverage,
        ...
        "final_population": stable_float_sum(final["peso"]),
