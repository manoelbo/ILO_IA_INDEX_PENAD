...
# ---------------------------------------------------------------------------
GCP_PROJECT_ID = (
    os.environ.get("REPLICATION_BILLING_PROJECT")
    or os.environ.get("GOOGLE_CLOUD_PROJECT")
)

# Período publicado: Jan/2021 — Jun/2025 (54 meses)
...
def download_bigquery_rapido(query, descricao=""):
    ...
    if not GCP_PROJECT_ID:
        raise RuntimeError(
            "Defina REPLICATION_BILLING_PROJECT ou GOOGLE_CLOUD_PROJECT "
            "antes de baixar o CAGED."
        )
...
def download_caged_ano(ano):
    ...
    end_month_filter = f"AND mes <= {MES_FIM}" if ano == ANO_FIM else ""
    query = f"""
...
def processar_ano(ano):
    ...
    if ano == ANO_FIM:
        ...
        df = df.loc[month.le(MES_FIM)].copy()
        log(
            f"  [{ano}] Jan–Jun: {len(df):,} registros "
            "(janela publicada até 2025-06)"
        )
