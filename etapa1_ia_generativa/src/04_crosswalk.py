...
def validate_crosswalk(df):
    ...
    if 'Profissionais das ciências' in exp_grupos.index:
        if exp_grupos['Profissionais das ciências'] > 0.30:
            logger.info("  ✓ Profissionais das ciências com exposição ALTA (correto)")
        else:
            logger.warning(f"  ⚠️ Profissionais das ciências com exposição BAIXA: {exp_grupos['Profissionais das ciências']:.3f}")
    
    if 'Ocupações elementares' in exp_grupos.index:
        if exp_grupos['Ocupações elementares'] < 0.20:
            logger.info("  ✓ Ocupações elementares com exposição BAIXA (correto)")
        else:
            logger.warning(f"  ⚠️ Ocupações elementares com exposição ALTA: {exp_grupos['Ocupações elementares']:.3f}")
...
if __name__ == "__main__":
    ...
    print("\n⚠️ IMPORTANTE: Revise o log antes de prosseguir!")
