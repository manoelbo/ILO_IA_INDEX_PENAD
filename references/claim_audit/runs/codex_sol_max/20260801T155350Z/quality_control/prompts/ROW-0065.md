# Blind second-review task: ROW-0065
...
- This prompt: `quality_control/prompts/ROW-0065.md`
...
- Candidate JSON only: `quality_control/candidates/ROW-0065.candidate.json`
- Exact evidence PDF only: `quality_control/evidence_pages/ROW-0065.pdf`
- Optional selected-page PNG renders only inside: `quality_control/renders/ROW-0065/`

Do not write the final `quality_control/results/ROW-0065.json`; the QC controller validates and commits it atomically.
...
```json
{
  ...
  "source_excerpt": "O ILO Global Index (Gmyrek et al., 2025), da OIT, é o único estruturado na ISCO-08, a classificação ocupacional internacional da ONU, complementada pela taxonomia polonesa de seis dígitos. Sua classificação combina validação humana e LLMs em três camadas: 1.640 trabalhadores avaliaram tarefas de sua própria área, um painel de especialistas revisou um subconjunto para avaliar a viabilidade prática da automação, e os modelos GPT-4o e Gemini Flash 1.5 atuaram como desempate nos casos de divergência. Esse gabarito validado por humanos foi então usado para escalar a classificação às 3.265 tarefas da ISCO-08.",
  "fact_checked": "",
  "pages": ""
}
```
...
- row_id: `ROW-0065`
- claim_row_sha256: `e2cafc1c46a89f392e55a15f18625820e0422c0e648515f5783b47f22f46cc81`
...
Extract only the exact supporting, conflicting, or diagnostic page range into `quality_control/evidence_pages/ROW-0065.pdf`. Use PyPDF2 with the physical one-based indices and preserve source pages without modification. The evidence file must exist even for a non-pass result.

Produce one independent JSON object conforming exactly to `quality_control/qc_result.schema.json`. Set:

- `schema_version`: `1.0.0`
- `row_id`: `ROW-0065`
- `worker_task_name`: `qc_row_0065`
...
- `source_sha256`: `9077761c72aabaa3c7ee70999ec5aa46a6afcf943b5fb01baa971f601d363ee9`
- `schema_sha256`: `e478b8113d3ed5fabc02f101e4746e9a627dd6657ecfeed2e86fbef8f9ca6dde`
- `evidence_path`: `quality_control/evidence_pages/ROW-0065.pdf`
