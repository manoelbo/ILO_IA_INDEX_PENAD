# Blind second-review task: ROW-0114
...
- This prompt: `quality_control/prompts/ROW-0114.md`
...
- Candidate JSON only: `quality_control/candidates/ROW-0114.candidate.json`
- Exact evidence PDF only: `quality_control/evidence_pages/ROW-0114.pdf`
- Optional selected-page PNG renders only inside: `quality_control/renders/ROW-0114/`

Do not write the final `quality_control/results/ROW-0114.json`; the QC controller validates and commits it atomically.
...
```json
{
  ...
  "affirmation_pt": "Os dados mensais da ADP permitem aos autores observar admissões, desligamentos e salários em alta frequência e quase em tempo real.",
  "source_excerpt": "Para a estratégia empírica, a principal referência foi a abordagem de diferenças em diferenças e de estudo de eventos aplicados à difusão da IA feita por Brynjolfsson, Chandar e Chen (2025). Esse estudo acompanhou o mercado de trabalho americano a partir dos registros de folha de pagamento da ADP, uma das maiores processadoras de folha dos Estados Unidos, que paga salários para mais de 25 milhões de trabalhadores. Como a empresa processa esses pagamentos mês a mês, os autores conseguem observar admissões, desligamentos e salários em altíssima frequência e quase em tempo real, montando um painel mensal de milhões de vínculos por firma.",
  "fact_checked": "",
  "pages": ""
}
```
...
- row_id: `ROW-0114`
- claim_row_sha256: `d9a5f02e76365a465bd28a5418055d9fdef5e8ea1c8e089e1df36f87a9f2ff3a`
...
Extract only the exact supporting, conflicting, or diagnostic page range into `quality_control/evidence_pages/ROW-0114.pdf`. Use PyPDF2 with the physical one-based indices and preserve source pages without modification. The evidence file must exist even for a non-pass result.

Produce one independent JSON object conforming exactly to `quality_control/qc_result.schema.json`. Set:

- `schema_version`: `1.0.0`
- `row_id`: `ROW-0114`
- `worker_task_name`: `qc_row_0114`
...
- `source_sha256`: `75012cdca09a734e64e6dd75e635551286549343dd7549252284dea9dc454a7d`
- `schema_sha256`: `e478b8113d3ed5fabc02f101e4746e9a627dd6657ecfeed2e86fbef8f9ca6dde`
- `evidence_path`: `quality_control/evidence_pages/ROW-0114.pdf`
