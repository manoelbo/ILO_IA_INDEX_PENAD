...
PYTHON_DEPS = WORKSPACE / "references/claim_audit/runs/codex_sol_max/20260801T155350Z/source_build/python_deps"
...
def preflight(timeout_seconds: int) -> None:
    receipt = QC_DIR / "runtime_preflight.txt"
    prompt = (
    ...
    text = receipt.read_text(encoding="utf-8").strip() if receipt.is_file() else ""
    if completed.returncode != 0 or text != "RUNTIME_OK gpt-5.6-sol max":
        raise RuntimeError(
            f"QC runtime preflight failed: exit={completed.returncode}, receipt={text!r}, "
            f"stderr={completed.stderr[-2000:]!r}"
        )
    log("qc_runtime_preflight_passed", model=MODEL, reasoning_effort=REASONING_EFFORT, receipt=text)
...
def process_row(row: dict[str, str], max_attempts: int, timeout_seconds: int) -> None:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        task_name = f"retry{attempt}_qc_row_{int(row_id.rsplit('-', 1)[1]):04d}"
...
def main() -> None:
    ...
    try:
        if not args.skip_preflight:
            preflight(args.timeout_seconds)
        for row in selected:
