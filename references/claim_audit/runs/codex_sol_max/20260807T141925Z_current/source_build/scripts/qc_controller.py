...
import current_controller as current
...
PRIMARY_SNAPSHOT_DIR = QC_DIR / "primary_snapshot"
CURRENT_INVENTORY_PATH = RUN_DIR / "claim_inventory_current.tsv"
CURRENT_AUDIT_PATH = RUN_DIR / "claim_inventory_audited_current.tsv"
DELTA_PATH = RUN_DIR / "claim_inventory_delta.tsv"
PRIOR_RUN_DIR = core.WORKSPACE / "references/claim_audit/runs/codex_sol_max/20260801T155350Z"

MODEL = "gpt-5.6-sol"
REASONING_EFFORT = "max"
SAMPLE_SEED = f"{core.RUN_ID}|REUSED_SUPPORTED_NO_CONCORDANT_QC|10pct"
SAMPLE_RATE = 0.10
...
def all_row_metadata() -> dict[str, dict[str, str]]:
    inventory = {row["row_id"]: row for row in read_tsv(CURRENT_INVENTORY_PATH)[1]}
    delta = {row["row_id"]: row for row in read_tsv(DELTA_PATH)[1]}
    bibliography = current.parse_bib_entries()
    metadata: dict[str, dict[str, str]] = {}
    for row_id, row in inventory.items():
        reconciliation = delta[row_id]
        source_manifest = json.loads(
            (RUN_DIR / f"source_build/{row['citation_key']}/source_build_manifest.json").read_text(
                encoding="utf-8"
            )
        )
        bib_entry = bibliography[row["citation_key"]]
        metadata[row_id] = {
            "row_id": row_id,
            ...
            "prior_row_id": reconciliation["prior_row_id"],
            "current_source_sha256": source_manifest["source_sha256"],
            "source_path": source_manifest["source_path"],
            "queue_status": "COMPLETED",
            "claim_row_sha256": reconciliation["claim_text_sha256"],
            "bibliography_entry_sha256": current.sha256_bytes(bib_entry.encode("utf-8")),
            "source_type": "LEGAL_HTML" if source_manifest.get("source_type") == "LEGAL_HTML" else "PDF",
            "worker_prompt_path": f"source_build/prompts/{row_id}.md",
            "worker_prompt_sha256": "",
            "worker_task_name": f"current_claim_row_{row_id.rsplit('-', 1)[1]}",
        }
    return metadata


def prior_qc_status(prior_row_id: str) -> str:
    if not prior_row_id:
        return "NOT_APPLICABLE"
    path = PRIOR_RUN_DIR / f"quality_control/comparisons/{prior_row_id}.json"
    if not path.is_file():
        return "NO_QC"
    return json.loads(path.read_text(encoding="utf-8")).get("comparison_status", "UNKNOWN")


def hydrate_primary_result(row: dict[str, str]) -> dict[str, Any]:
    current_path = core.RESULTS_DIR / f"{row['row_id']}.json"
    result = json.loads(current_path.read_text(encoding="utf-8"))
    if "confidence" not in result:
        prior_row_id = row["prior_row_id"]
        if not prior_row_id:
            raise SystemExit(f"Minimal reused result lacks prior_row_id: {row['row_id']}")
        prior_path = PRIOR_RUN_DIR / f"results/{prior_row_id}.json"
        if not prior_path.is_file():
            raise SystemExit(f"Prior result missing for reused row: {row['row_id']}")
        hydrated = copy.deepcopy(json.loads(prior_path.read_text(encoding="utf-8")))
        hydrated.update(
            {
                "row_id": row["row_id"],
                "claim_id": row["claim_id"],
                ...
                "pages": result["pages"],
                "printed_pages": result["pages"],
                "schema_sha256": current.sha256_file(current.SCHEMA_PATH),
            }
        )
        for issue in hydrated.get("issues", []):
            issue["row_id"] = row["row_id"]
            issue["evidence_path"] = result["evidence_path"]
        result = hydrated
    result["reconciliation_status"] = row["reconciliation_status"]
    result["prior_row_id"] = row["prior_row_id"]
    result["prior_qc_status"] = prior_qc_status(row["prior_row_id"])
    return result


def load_primary_results() -> list[dict[str, Any]]:
    metadata = all_row_metadata()
    paths = sorted(core.RESULTS_DIR.glob("ROW-CUR-*.json"))
    if len(paths) != len(metadata):
        raise SystemExit(f"Expected {len(metadata)} primary results before QC; found {len(paths)}")
    results = [hydrate_primary_result(metadata[path.stem]) for path in paths]
    expected = [f"ROW-CUR-{index:04d}" for index in range(1, len(metadata) + 1)]
    actual = [result.get("row_id") for result in results]
    if actual != expected:
        raise SystemExit("Primary results do not form the exact current ROW-CUR sequence")
    PRIMARY_SNAPSHOT_DIR.mkdir(parents=True, exist_ok=True)
    for result in results:
        core.atomic_write_json(PRIMARY_SNAPSHOT_DIR / f"{result['row_id']}.json", result)
    return results
    ...
    snapshot = [
        {
            ...
            "sha256": core.sha256_file(PRIMARY_SNAPSHOT_DIR / f"{result['row_id']}.json"),
        }
        for result in results
    ]
...
def verify_qc_invariants(checkpoint: str) -> None:
    ...
    for row in rows:
        primary_path = PRIMARY_SNAPSHOT_DIR / f"{row['row_id']}.json"
...
def build_qc_schema() -> dict[str, Any]:
    ...
    evidence_pattern = r"^quality_control/evidence_pages/ROW-CUR-[0-9]{4}\.(pdf|html)$"
...
def supported_sample(results: list[dict[str, Any]]) -> list[str]:
    supported = [
        result
        for result in results
        if result["reconciliation_status"] == "REUSED_EXACT"
        and result["fact_checked"] == "SUPPORTED"
        and result["prior_qc_status"] != "AGREES"
    ]
...
def selection_reasons(result: dict[str, Any], sampled: set[str]) -> list[str]:
    reasons: list[str] = []
    if result["reconciliation_status"] != "REUSED_EXACT":
        reasons.append("CURRENT_NEW_OR_CHANGED")
    ...
    if result.get("prior_qc_status") == "NEEDS_ADJUDICATION":
        reasons.append("PRIOR_QC_DISAGREEMENT")
    if result["row_id"] in sampled:
        reasons.append("DETERMINISTIC_REUSED_SUPPORTED_SAMPLE")
    return reasons
...
def cmd_init() -> None:
    ...
    eligible_sample_population = sum(
        result["reconciliation_status"] == "REUSED_EXACT"
        and result["fact_checked"] == "SUPPORTED"
        and result["prior_qc_status"] != "AGREES"
        for result in results
    )
    if len(sampled_rows) != math.ceil(eligible_sample_population * SAMPLE_RATE):
    ...
    for sequence, (result, reasons) in enumerate(selected, start=1):
        ...
        primary_result_path = PRIMARY_SNAPSHOT_DIR / f"{row_id}.json"
        ...
        queue_rows.append(
            {
                ...
                "primary_prompt_sha256": (
                    core.sha256_file(RUN_DIR / primary["worker_prompt_path"])
                    if (RUN_DIR / primary["worker_prompt_path"]).is_file()
                    else "NOT_CREATED_FOR_REUSED_EXACT"
                ),
    ...
    reason_counts = {
        ...
        for reason in (
            "CURRENT_NEW_OR_CHANGED",
            "NON_SUPPORTED",
            "LOW_OR_MEDIUM_CONFIDENCE",
            "SOURCE_VERSION_ISSUE",
            "PRIOR_QC_DISAGREEMENT",
            "DETERMINISTIC_REUSED_SUPPORTED_SAMPLE",
        )
    }
    selection_manifest = {
        "version": "1.0.0",
        ...
        "sample_algorithm": "Among REUSED_EXACT SUPPORTED rows without prior concordant QC, sort by SHA-256(seed + '|' + row_id), then take ceil(N * 0.10)",
        "sample_seed": SAMPLE_SEED,
        "sample_rate": SAMPLE_RATE,
        "supported_population": sum(
            result["reconciliation_status"] == "REUSED_EXACT"
            and result["fact_checked"] == "SUPPORTED"
            and result["prior_qc_status"] != "AGREES"
            for result in results
...
def transform_primary_prompt(row_id: str, task_name: str) -> str:
    ...
    if primary_prompt_path.is_file():
        text = primary_prompt_path.read_text(encoding="utf-8")
    else:
        metadata = all_row_metadata()[row_id]
        bibliography = current.parse_bib_entries()[metadata["citation_key"]]
        source_manifest = json.loads(
            (RUN_DIR / f"source_build/{metadata['citation_key']}/source_build_manifest.json").read_text(
                encoding="utf-8"
            )
        )
        text = current.prompt_for(metadata, bibliography, source_manifest)
    ...
    suffix = "html" if "official legal HTML source" in text else "pdf"

    text = text.replace(f"# Isolated current claim audit: {row_id}", f"# Blind second-review task: {row_id}")
...
def cmd_make_prompt(row_id: str) -> None:
    ...
    task_name = f"qc_row_{int(row_id.rsplit('-', 1)[1]):04d}"
...
def validate_qc_result(result: dict[str, Any], selected: dict[str, str]) -> None:
    ...
    primary = json.loads((PRIMARY_SNAPSHOT_DIR / f"{selected['row_id']}.json").read_text(encoding="utf-8"))
...
def compare_assessments(primary: dict[str, Any], qc: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "primary_result_sha256": core.sha256_file(PRIMARY_SNAPSHOT_DIR / f"{primary['row_id']}.json"),
...
def cmd_rebuild_comparisons() -> None:
    ...
    for row in rows:
        ...
        if not result_path.is_file():
            continue
        primary = json.loads((PRIMARY_SNAPSHOT_DIR / f"{row_id}.json").read_text(encoding="utf-8"))
